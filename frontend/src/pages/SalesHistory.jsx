...
import CancelIcon from '@mui/icons-material/Cancel'; // Icono Anular
...
function SalesHistory() {
  ...
  useEffect(() => {
    ...
    if (searchText) {
      ...
      result = result.filter(sale => 
        sale.sale_id.toString().includes(lower) || 
        (sale.client_dni && sale.client_dni.includes(lower))
      );
    }

    // 2. Método de Pago
    if (filterMethod !== "TODOS") {
      result = result.filter(sale => sale.payment_method === filterMethod);
    }

    // 3. Usuario
    if (filterUser !== "TODOS") {
      result = result.filter(sale => sale.user_name === filterUser);
    }

    // 4. Fechas
    if (startDate) {
      result = result.filter(sale => new Date(sale.date_created) >= new Date(startDate));
    }
    if (endDate) {
      const end = new Date(endDate);
      end.setHours(23, 59, 59);
      result = result.filter(sale => new Date(sale.date_created) <= end);
    }

    setFilteredSales(result);
  }, [searchText, filterMethod, filterUser, startDate, endDate, sales]);
  ...
  const fetchSales = async () => {
    ...
    try {
      ...
      const methods = [...new Set(data.map(s => s.payment_method))];
      const users = [...new Set(data.map(s => s.user_name))];
  ...
  const getStatusChip = (status, type) => {
    ...
    return <Chip label={status || 'PENDIENTE'} color="default" size="small" variant="outlined" />;
  };
    ...
    <Layout>
        ...
        <Grid container spacing={2} alignItems="center">
          ...
          <Grid item xs={6} md={2}>
            ...
            >
              ...
              {uniqueMethods.map(m => <MenuItem key={m} value={m}>{m}</MenuItem>)}
            </TextField>
          </Grid>
          ...
          <Grid item xs={6} md={2}>
            <TextField
              select fullWidth label="Cajero" size="small"
              ...
              {filteredSales.length === 0 ? (
                ...
                filteredSales.map((sale) => (
                  <TableRow key={sale.sale_id} hover>
                    ...
                    <TableCell>{sale.client_dni || "Público General"}</TableCell>
                    <TableCell>
                      <Chip label={sale.payment_method} size="small" variant="outlined" sx={{ color: 'text.primary', borderColor: 'divider' }} />
                    </TableCell>
                    <TableCell sx={{ fontWeight: 'bold', color: 'success.main' }}>S/ {sale.total_amount.toFixed(2)}</TableCell>
                    <TableCell>
                    ...
                    <TableCell align="center">
                      ...
                      {/* Botón ANULAR: Solo si es ACEPTADO y NO es NC */}
      ...
      <Dialog open={modalOpen} onClose={() => setModalOpen(false)} fullWidth maxWidth="sm">
          ...
          {selectedSale && (
            <Box>
              ...
              <Box sx={{ display: 'flex', justifyContent: 'space-between', mt: 3, pt: 2, borderTop: '1px dashed', borderColor: 'text.secondary' }}>
                ...
                <Typography variant="h5" color="success.main" fontWeight="bold">S/ {selectedSale.total_amount.toFixed(2)}</Typography>
              </Box>
        ...
        <DialogActions sx={{ p: 2, bgcolor: 'background.paper' }}>
          ...
          <Button variant="contained" startIcon={<PrintIcon />} onClick={handlePrintPdf}>
            REIMPRIMIR TICKET
          </Button>
        </DialogActions>
      </Dialog>
